; Listing generated by Microsoft (R) Optimizing Compiler Version 19.16.27026.1 

include listing.inc

INCLUDELIB MSVCRTD
INCLUDELIB OLDNAMES

msvcjmc	SEGMENT
__7B7A869E_ctype@h DB 01H
__457DD326_basetsd@h DB 01H
__4384A2D9_corecrt_memcpy_s@h DB 01H
__4E51A221_corecrt_wstring@h DB 01H
__2140C079_string@h DB 01H
__1887E595_winnt@h DB 01H
__9FC7C64B_processthreadsapi@h DB 01H
__FA470AEC_memoryapi@h DB 01H
__F37DAFF1_winerror@h DB 01H
__7A450CCC_winbase@h DB 01H
__B4B40122_winioctl@h DB 01H
__86261D59_stralign@h DB 01H
__059414E1_pmc_sint_debug@h DB 01H
__843F7E1A_test_op_getnumbertype@c DB 01H
msvcjmc	ENDS
PUBLIC	TEST_GetNumberType_X
PUBLIC	__JustMyCode_Default
PUBLIC	??_C@_1EC@NPMCAPKO@?$AAF?$AAr?$AAo?$AAm?$AAB?$AAy?$AAt?$AAe?$AAA?$AAr?$AAr?$AAa?$AAy?$AAn?$PP?$KJ@ ; `string'
PUBLIC	??_C@_1DK@LCFBHAOB@?$AAT?$AAE?$AAS?$AAT?$AA_?$AAG?$AAe?$AAt?$AAN?$AAu?$AAm?$AAb?$AAe?$AAr?$AAT@ ; `string'
PUBLIC	??_C@_1FA@PFINNHIK@?$AAG?$AAe?$AAt?$AAP?$AAr?$AAo?$AAp?$AAe?$AAr?$AAt?$AAy?$AAV?$AAa?$AAl?$AAu@ ; `string'
PUBLIC	??_C@_1BK@CBDJCOBM@?$PP?G?$PP?$PM?$PP?$LP?$AAn?$PP?$IF?$PP?$LJ?$AAL?$AA?$AA?$PP?t?$AAW?$AAj?$AAD@ ; `string'
EXTRN	TEST_Assert:PROC
EXTRN	FormatTestLabel:PROC
EXTRN	FormatTestMesssage:PROC
EXTRN	_RTC_CheckStackVars:PROC
EXTRN	_RTC_InitBase:PROC
EXTRN	_RTC_Shutdown:PROC
EXTRN	__CheckForDebuggerJustMyCode:PROC
EXTRN	__GSHandlerCheck:PROC
EXTRN	__security_check_cookie:PROC
EXTRN	__security_cookie:QWORD
;	COMDAT pdata
pdata	SEGMENT
$pdata$TEST_GetNumberType_X DD imagerel $LN10
	DD	imagerel $LN10+506
	DD	imagerel $unwind$TEST_GetNumberType_X
pdata	ENDS
;	COMDAT rtc$TMZ
rtc$TMZ	SEGMENT
_RTC_Shutdown.rtc$TMZ DQ FLAT:_RTC_Shutdown
rtc$TMZ	ENDS
;	COMDAT rtc$IMZ
rtc$IMZ	SEGMENT
_RTC_InitBase.rtc$IMZ DQ FLAT:_RTC_InitBase
rtc$IMZ	ENDS
;	COMDAT ??_C@_1BK@CBDJCOBM@?$PP?G?$PP?$PM?$PP?$LP?$AAn?$PP?$IF?$PP?$LJ?$AAL?$AA?$AA?$PP?t?$AAW?$AAj?$AAD@
CONST	SEGMENT
??_C@_1BK@CBDJCOBM@?$PP?G?$PP?$PM?$PP?$LP?$AAn?$PP?$IF?$PP?$LJ?$AAL?$AA?$AA?$PP?t?$AAW?$AAj?$AAD@ DB 0c7H
	DB	'0', 0fcH, '0', 0bfH, '0n0', 085H, 'Q', 0b9H, '[L0', 00H, 'N', 0f4H
	DB	081H, 'W0j0D0', 00H, 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_1FA@PFINNHIK@?$AAG?$AAe?$AAt?$AAP?$AAr?$AAo?$AAp?$AAe?$AAr?$AAt?$AAy?$AAV?$AAa?$AAl?$AAu@
CONST	SEGMENT
??_C@_1FA@PFINNHIK@?$AAG?$AAe?$AAt?$AAP?$AAr?$AAo?$AAp?$AAe?$AAr?$AAt?$AAy?$AAV?$AAa?$AAl?$AAu@ DB 'G'
	DB	00H, 'e', 00H, 't', 00H, 'P', 00H, 'r', 00H, 'o', 00H, 'p', 00H
	DB	'e', 00H, 'r', 00H, 't', 00H, 'y', 00H, 'V', 00H, 'a', 00H, 'l'
	DB	00H, 'u', 00H, 'e', 00H, '_', 00H, 'X', 00H, '_', 00H, 'I', 00H
	DB	'n0', 0a9H, '_0^', 0b3H, '0', 0fcH, '0', 0c9H, '0L0', 01fH, 'g'
	DB	085H, '_', 01aH, 090H, 08aH, '0g0o0j0D0(', 00H, '%', 00H, 'd', 00H
	DB	')', 00H, 00H, 00H				; `string'
CONST	ENDS
;	COMDAT ??_C@_1DK@LCFBHAOB@?$AAT?$AAE?$AAS?$AAT?$AA_?$AAG?$AAe?$AAt?$AAN?$AAu?$AAm?$AAb?$AAe?$AAr?$AAT@
CONST	SEGMENT
??_C@_1DK@LCFBHAOB@?$AAT?$AAE?$AAS?$AAT?$AA_?$AAG?$AAe?$AAt?$AAN?$AAu?$AAm?$AAb?$AAe?$AAr?$AAT@ DB 'T'
	DB	00H, 'E', 00H, 'S', 00H, 'T', 00H, '_', 00H, 'G', 00H, 'e', 00H
	DB	't', 00H, 'N', 00H, 'u', 00H, 'm', 00H, 'b', 00H, 'e', 00H, 'r'
	DB	00H, 'T', 00H, 'y', 00H, 'p', 00H, 'e', 00H, '_', 00H, 'X', 00H
	DB	' ', 00H, '(', 00H, '%', 00H, 'd', 00H, '.', 00H, '%', 00H, 'd'
	DB	00H, ')', 00H, 00H, 00H			; `string'
CONST	ENDS
;	COMDAT ??_C@_1EC@NPMCAPKO@?$AAF?$AAr?$AAo?$AAm?$AAB?$AAy?$AAt?$AAe?$AAA?$AAr?$AAr?$AAa?$AAy?$AAn?$PP?$KJ@
CONST	SEGMENT
??_C@_1EC@NPMCAPKO@?$AAF?$AAr?$AAo?$AAm?$AAB?$AAy?$AAt?$AAe?$AAA?$AAr?$AAr?$AAa?$AAy?$AAn?$PP?$KJ@ DB 'F'
	DB	00H, 'r', 00H, 'o', 00H, 'm', 00H, 'B', 00H, 'y', 00H, 't', 00H
	DB	'e', 00H, 'A', 00H, 'r', 00H, 'r', 00H, 'a', 00H, 'y', 00H, 'n'
	DB	'0', 0a9H, '_0^', 0b3H, '0', 0fcH, '0', 0c9H, '0L0', 01fH, 'g', 085H
	DB	'_', 01aH, 090H, 08aH, '0g0o0j0D0(', 00H, '%', 00H, 'd', 00H, ')'
	DB	00H, 00H, 00H				; `string'
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$TEST_GetNumberType_X DD 025054a19H
	DD	011d2322H
	DD	07016002fH
	DD	05015H
	DD	imagerel __GSHandlerCheck
	DD	0160H
xdata	ENDS
;	COMDAT CONST
CONST	SEGMENT
TEST_GetNumberType_X$rtcName$0 DB 078H
	DB	00H
	ORG $+6
TEST_GetNumberType_X$rtcName$1 DB 061H
	DB	063H
	DB	074H
	DB	075H
	DB	061H
	DB	06cH
	DB	05fH
	DB	076H
	DB	061H
	DB	06cH
	DB	075H
	DB	065H
	DB	00H
	ORG $+11
TEST_GetNumberType_X$rtcVarDesc DD 044H
	DD	04H
	DQ	FLAT:TEST_GetNumberType_X$rtcName$1
	DD	028H
	DD	08H
	DQ	FLAT:TEST_GetNumberType_X$rtcName$0
	ORG $+96
TEST_GetNumberType_X$rtcFrameData DD 02H
	DD	00H
	DQ	FLAT:TEST_GetNumberType_X$rtcVarDesc
CONST	ENDS
; Function compile flags: /Odt
;	COMDAT __JustMyCode_Default
_TEXT	SEGMENT
__JustMyCode_Default PROC				; COMDAT
	ret	0
__JustMyCode_Default ENDP
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File z:\sources\lunor\repos\rougemeilland\palmtree.math.core.sint\palmtree.math.core.sint\test_op_getnumbertype.c
;	COMDAT TEST_GetNumberType_X
_TEXT	SEGMENT
x$ = 8
actual_value$ = 36
result$ = 68
x_result$ = 100
tv133 = 308
tv91 = 308
tv74 = 308
tv82 = 312
tv64 = 312
__$ArrayPad$ = 320
env$ = 368
ep$ = 376
no$ = 384
x_buf$ = 392
x_buf_size$ = 400
desired_value$ = 408
TEST_GetNumberType_X PROC				; COMDAT

; 33   : {

$LN10:
	mov	QWORD PTR [rsp+32], r9
	mov	DWORD PTR [rsp+24], r8d
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+8], rcx
	push	rbp
	push	rdi
	sub	rsp, 376				; 00000178H
	lea	rbp, QWORD PTR [rsp+32]
	mov	rdi, rsp
	mov	ecx, 94					; 0000005eH
	mov	eax, -858993460				; ccccccccH
	rep stosd
	mov	rcx, QWORD PTR [rsp+408]
	mov	rax, QWORD PTR __security_cookie
	xor	rax, rbp
	mov	QWORD PTR __$ArrayPad$[rbp], rax
	lea	rcx, OFFSET FLAT:__843F7E1A_test_op_getnumbertype@c
	call	__CheckForDebuggerJustMyCode

; 34   :     PMC_HANDLE_SINT x;
; 35   :     PMC_NUMBER_TYPE_CODE actual_value;
; 36   :     PMC_STATUS_CODE result;
; 37   :     PMC_STATUS_CODE x_result;
; 38   :     TEST_Assert(env, FormatTestLabel(L"TEST_GetNumberType_X (%d.%d)", no, 1), (x_result = ep->FromByteArray(x_buf, x_buf_size, &x)) == PMC_STATUS_OK, FormatTestMesssage(L"FromByteArrayの復帰コードが期待通りではない(%d)", x_result));

	lea	r8, QWORD PTR x$[rbp]
	mov	rdx, QWORD PTR x_buf_size$[rbp]
	mov	rcx, QWORD PTR x_buf$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+608]
	mov	DWORD PTR x_result$[rbp], eax
	cmp	DWORD PTR x_result$[rbp], 0
	jne	SHORT $LN4@TEST_GetNu
	mov	DWORD PTR tv74[rbp], 1
	jmp	SHORT $LN5@TEST_GetNu
$LN4@TEST_GetNu:
	mov	DWORD PTR tv74[rbp], 0
$LN5@TEST_GetNu:
	mov	edx, DWORD PTR x_result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_1EC@NPMCAPKO@?$AAF?$AAr?$AAo?$AAm?$AAB?$AAy?$AAt?$AAe?$AAA?$AAr?$AAr?$AAa?$AAy?$AAn?$PP?$KJ@
	call	FormatTestMesssage
	mov	QWORD PTR tv64[rbp], rax
	mov	r8d, 1
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_1DK@LCFBHAOB@?$AAT?$AAE?$AAS?$AAT?$AA_?$AAG?$AAe?$AAt?$AAN?$AAu?$AAm?$AAb?$AAe?$AAr?$AAT@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv64[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv74[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 39   :     TEST_Assert(env, FormatTestLabel(L"TEST_GetNumberType_X (%d.%d)", no, 2), (result = ep->GetNumberType_X(x, &actual_value)) == PMC_STATUS_OK, FormatTestMesssage(L"GetPropertyValue_X_Iの復帰コードが期待通りではない(%d)", result));

	lea	rdx, QWORD PTR actual_value$[rbp]
	mov	rcx, QWORD PTR x$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+592]
	mov	DWORD PTR result$[rbp], eax
	cmp	DWORD PTR result$[rbp], 0
	jne	SHORT $LN6@TEST_GetNu
	mov	DWORD PTR tv91[rbp], 1
	jmp	SHORT $LN7@TEST_GetNu
$LN6@TEST_GetNu:
	mov	DWORD PTR tv91[rbp], 0
$LN7@TEST_GetNu:
	mov	edx, DWORD PTR result$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_1FA@PFINNHIK@?$AAG?$AAe?$AAt?$AAP?$AAr?$AAo?$AAp?$AAe?$AAr?$AAt?$AAy?$AAV?$AAa?$AAl?$AAu@
	call	FormatTestMesssage
	mov	QWORD PTR tv82[rbp], rax
	mov	r8d, 2
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_1DK@LCFBHAOB@?$AAT?$AAE?$AAS?$AAT?$AA_?$AAG?$AAe?$AAt?$AAN?$AAu?$AAm?$AAb?$AAe?$AAr?$AAT@
	call	FormatTestLabel
	mov	rcx, QWORD PTR tv82[rbp]
	mov	r9, rcx
	mov	r8d, DWORD PTR tv91[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 40   :     TEST_Assert(env, FormatTestLabel(L"TEST_GetNumberType_X (%d.%d)", no, 3), actual_value == desired_value, L"データの内容が一致しない");

	mov	eax, DWORD PTR desired_value$[rbp]
	cmp	DWORD PTR actual_value$[rbp], eax
	jne	SHORT $LN8@TEST_GetNu
	mov	DWORD PTR tv133[rbp], 1
	jmp	SHORT $LN9@TEST_GetNu
$LN8@TEST_GetNu:
	mov	DWORD PTR tv133[rbp], 0
$LN9@TEST_GetNu:
	mov	r8d, 3
	mov	edx, DWORD PTR no$[rbp]
	lea	rcx, OFFSET FLAT:??_C@_1DK@LCFBHAOB@?$AAT?$AAE?$AAS?$AAT?$AA_?$AAG?$AAe?$AAt?$AAN?$AAu?$AAm?$AAb?$AAe?$AAr?$AAT@
	call	FormatTestLabel
	lea	r9, OFFSET FLAT:??_C@_1BK@CBDJCOBM@?$PP?G?$PP?$PM?$PP?$LP?$AAn?$PP?$IF?$PP?$LJ?$AAL?$AA?$AA?$PP?t?$AAW?$AAj?$AAD@
	mov	r8d, DWORD PTR tv133[rbp]
	mov	rdx, rax
	mov	rcx, QWORD PTR env$[rbp]
	call	TEST_Assert

; 41   :     if (x_result == PMC_STATUS_OK)

	cmp	DWORD PTR x_result$[rbp], 0
	jne	SHORT $LN2@TEST_GetNu

; 42   :         ep->Dispose(x);

	mov	rcx, QWORD PTR x$[rbp]
	mov	rax, QWORD PTR ep$[rbp]
	call	QWORD PTR [rax+584]
$LN2@TEST_GetNu:

; 43   : }

	lea	rcx, QWORD PTR [rbp-32]
	lea	rdx, OFFSET FLAT:TEST_GetNumberType_X$rtcFrameData
	call	_RTC_CheckStackVars
	mov	rcx, QWORD PTR __$ArrayPad$[rbp]
	xor	rcx, rbp
	call	__security_check_cookie
	lea	rsp, QWORD PTR [rbp+344]
	pop	rdi
	pop	rbp
	ret	0
TEST_GetNumberType_X ENDP
_TEXT	ENDS
END
